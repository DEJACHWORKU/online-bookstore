...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($message)) {
        $check_stmt = $conn->prepare("SELECT id FROM books WHERE title = ? AND description = ? AND date = ? AND author = ?");
        $check_stmt->bind_param("ssss", $book_title, $book_description, $book_date, $author);
        $check_stmt->execute();
        $check_stmt->store_result();
        
        if ($check_stmt->num_rows > 0) {
            $message .= "<p class='error'>A book with the same title, description, date, and author already exists.</p>";
        }
        $check_stmt->close();
    }
    ...
    if (empty($message)) {
        ...
        $cover_filename = uniqid() . '.' . pathinfo($book_cover['name'], PATHINFO_EXTENSION);
        $file_filename = uniqid() . '.' . pathinfo($book_file['name'], PATHINFO_EXTENSION);
        
        $cover_path = $cover_dir . $cover_filename;
        $file_path = $file_dir . $file_filename;

        if (move_uploaded_file($book_cover['tmp_name'], $cover_path) && move_uploaded_file($book_file['tmp_name'], $file_path)) {
...
<body>
    ...
    <div class="container">
        <form id="book-form" method="post" enctype="multipart/form-data">
            ...
            <label for="book_cover">Book Cover (PNG/JPG):</label>
            <input type="file" id="book_cover" name="book_cover" accept="image/png, image/jpeg" required>
